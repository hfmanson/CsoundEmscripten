...
var CsoundObj = function() {
	...
	var _compileOrcSco = cwrap('CsoundObj_compileOrcSco', null, ['number', 'string', 'string']);
	...
	var _start = cwrap('CsoundObj_start', ['number'], ['number']);
	...
	this.compileOrcSco = function(fileOrc, fileSco) {

		_compileOrcSco(_self, fileOrc, fileSco);
		compiled = true;
	};
	...
	this.compileOrc = function(orchestraString) {

		return _compileOrc(_self, orchestraString);
	};
...
var counter = 0;
		...
		_start(_self);
		audioProcessNode.connect(audioContext.destination);1
